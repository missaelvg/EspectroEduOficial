// scripts/dashboard_views.js (VERSIÓN FINAL: CRUCIGRAMA Y QUIZ ALEATORIOS)
...
// Selecciona preguntas únicas para el estudiante
...
// NUEVO: Selecciona palabras únicas para el crucigrama del estudiante
function getStudentCrosswordWords(allWords, studentUid, count = 6) {
    if (!allWords || allWords.length === 0) return [];
    let seed = 0;
    // Usamos una semilla ligeramente diferente para que no sea idéntico patrón al quiz
    for (let i = 0; i < studentUid.length; i++) seed += studentUid.charCodeAt(i) + 5;
    ...
    try {
        ...
        if (list.length === 0) html = `<div class="card"><p>No hay prácticas creadas.</p></div>`;
            ...
            list.forEach(p => {
                ...
                const mBtn = p.manualPdfUrl?.length > 5 ? `<a href="${p.manualPdfUrl}" target="_blank" class="btn btn-secondary">Ver Manual</a>` : '';
                html += `<div class="card"><div style="display:flex; justify-content:space-between; align-items:center;"><h4>${p.title}</h4><button onclick="handleDeletePractice('${p.id}')" style="background-color:#ef4444; font-size:0.8em; padding:6px 12px; border:none; color:white;">Borrar</button></div><p style="color:#64748b;">${count} alumnos</p><div style="display:flex; gap:10px; flex-wrap:wrap;">${sBtn}${stdBtn}${mBtn}</div></div>`;
            });
        }
...
async function handlePracticeCreation() {
    ...
    try {
        ...
        if (txt.length > 50) {
            logDiv.innerHTML = '<span style="color:#eab308">5/5: Creando banco de preguntas y palabras...</span>';
...
// ... (Gestión Alumnos y Notas Doctor - Igual) ...
...
// --- CRUCIGRAMA (ALEATORIO ÚNICO + NAVEGACIÓN) ---
function renderCrossword(p) {
    ...
    // 1. Seleccionar 6 palabras únicas del banco
    const myWordsData = getStudentCrosswordWords(allWordsData, AppState.user.uid, 6);
    
    // 2. Mapear y Generar Layout
    const words = myWordsData.map(w => ({ word: w.word.toUpperCase(), clue: w.clue }));
    const layout = generateCrosswordLayout(words);
    if(!layout) return d.innerHTML="<p>Error generando crucigrama (intenta recargar).</p>";
        ...
        r.forEach((c, j) => { 
            if(c) {
                ...
                h += `<td style="position:relative;">
                        <input type="text" maxlength="1" data-correct="${c.char}" class="crossword-cell" id="${cellId}" data-p="${p.id}" data-r="${i}" data-c="${j}">
                        <span class="crossword-number">${c.num||''}</span>
                      </td>`;
    ...
    d.querySelectorAll('.crossword-cell').forEach(input => {
        input.addEventListener('keydown', (e) => {
            ...
            let nextId = null;
            switch(e.key) {
                case 'ArrowUp':    nextId = `cell-${pid}-${r-1}-${c}`; break;
                case 'ArrowDown':  nextId = `cell-${pid}-${r+1}-${c}`; break;
                case 'ArrowLeft':  nextId = `cell-${pid}-${r}-${c-1}`; break;
                case 'ArrowRight': nextId = `cell-${pid}-${r}-${c+1}`; break;
            }
            if (nextId) {
