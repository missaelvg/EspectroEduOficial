// scripts/dashboard_views.js (VERSIÓN FINAL: CRUCIGRAMA CON NAVEGACIÓN POR TECLADO)
...
function renderCrossword(p) {
    ...
    let h = '<table>';
    // CORRECCIÓN: Agregar 'i' y 'j' para los IDs y coordenadas
    layout.grid.forEach((r, i) => { 
        h += '<tr>'; 
        r.forEach((c, j) => { 
            if(c) {
                // ID único: cell-PRACTICE_ID-ROW-COL
    ...
    // Agregar estilos locales para números
    const st = document.createElement('style'); 
    st.innerHTML = `.crossword-number{position:absolute;top:1px;left:1px;font-size:8px;color:#333;}`; 
    d.appendChild(st);

    // --- ACTIVAR NAVEGACIÓN POR TECLADO ---
    const inputs = d.querySelectorAll('.crossword-cell');
    inputs.forEach(input => {
        input.addEventListener('keydown', (e) => {
            const r = parseInt(input.dataset.r);
            const c = parseInt(input.dataset.c);
            const pid = input.dataset.p;
            let nextId = null;
